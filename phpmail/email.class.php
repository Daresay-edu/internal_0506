...

{
...

	{
		...
		$this->time_out = 3330; //is used in fsockopen() 
#
...
#
...

	{
		...
		if (count($this->attachments) > 0) {
            $header .= "Content-Type: multipart/mixed;" . EOL . chr(9) . " boundary=\"" . $bndp . "\"" . EOL . EOL;
            $header .= '--'.$bndp . EOL;
            $header .= 'Content-Type : multipart/alternative; boundary="' . $bnd . '"' . EOL . EOL;
            $header .= '--' . $bnd . EOL;
