<ul>
	<li><a href="#"><span>管理</span></a><span class="dropBottom"></span>
		<ul>
			<li><a href="#">课程管理</a><span class="dropRight"></span>
	
				<ul>
					<li><a href="mail_index.php" target="">备课</a></li>
					<li><a href="lesson_record_index.php" target="">已上课程记录</a></li>
				</ul>
			</li>
			<li><a href="school_add.php" target="">校区管理</a></li>
			<li><a href="class_add.php" target="">班级管理</a></li>
			<li><a href="#">教师管理</a>
				<ul>
					<li><a href="teacher_salary_record.php" target="">教师打卡</a></li>
					<li><a href="teacher_addh.php" target="">教师管理</a></li>
				</ul>
			</li>
			<li><a href="#">学生管理</a>
				<ul>
					<li><a href="demo_student_add.php" target="">试听</a></li>
					<li><a href="students_index.php" target="">学生管理</a></li>
					<li><a href="absent_index.php" target="">出勤</a></li>
					<li><a href="dollar_add.php" target="">Dollar</a></li>
					<li><a href="students_credit_index.php" target="">积分</a></li>
				</ul>
			</li>
			<li><a href="#">教务和运营</a>
				<ul>
					<li><a href="attendance_index.php" target="">出勤表管理</a></li>
					<li><a href="remind_index.php" target="">提醒/通知</a></li>
					<li><a href="income_stat.php" target="">收入</a></li>
					<li><a href="https://www-k6.thinkcentral.com/ePCEval/evalLogin.do" target="">LA/SF Online</a></li>
				</ul>
			</li>
		</ul>
	</li>
</ul>

